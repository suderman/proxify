...
module.exports = function (command) {
  ...
  var argv = require('yargs')(command).usage('Usage: proxify (<hostname> | <file>) [:listen] [:target] [--options]...')
  // .example('$0 count -f foo.js', 'count the lines in the given file')
  .alias('d', 'DOMAIN').describe('d', 'Domain name sub-domains built on')['default']('d', process.env.DOMAIN || 'local').alias('i', 'SERVER').describe('i', 'Private IP address of the host server')['default']('i', process.env.SERVER || '127.0.0.1').alias('o', 'ROUTER').describe('o', 'Domain pointing to home router')['default']('o', process.env.ROUTER || 'router.' + (process.env.DOMAIN || 'local')).alias('c', 'CA').describe('c', 'Certificate Authority URL')['default']('c', process.env.CA).alias('n', 'DNSIMPLE').describe('n', 'email:key')['default']('n', process.env.DNSIMPLE).alias('s', 'ssl').describe('s', 'Create SSL host & certificate').boolean('s').alias('a', 'auth').describe('a', 'Authenticate with client certificate (ssl implied)').boolean('a').alias('p', 'password').describe('p', 'Authenticate with username/password (ssl implied)').boolean('p').alias('r', 'redirect').describe('r', 'Redirect to target').boolean('r').help('h').alias('h', 'help').demand(1).argv;
